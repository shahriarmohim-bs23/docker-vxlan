...
def main():
    ...
    #print(image)
    client.containers.run("ubuntu-docker:latest", name={dockername}, network_mode=None,detach=True)
    BR="br"+i
    ...
    ETH="eth0"
    IP = "192.168."+i+".11/24"
    command = ["sudo","ovs-docker","add-port",BR,ETH,dockername,"--ipaddress="+IP," --gateway="+"192.168."+i+".1"]
    run_command(command)
    print(command)
    #sudo ovs-vsctl add-port ovs-br0 vxlan0 -- set interface vxlan0 type=vxlan options:remote_ip=10.0.1.169 options:key=1000
    VXLAN="vxlan"+i
    VNI=i
    command=["sudo","ovs-vsctl","add-port",BR,VXLAN,"--set","interface",VXLAN,"type=vxlan","options:remote_ip=10.0.1.43","options:key="+VNI]
    run_command(command)
    print(command)
